...
export default function CasoCard({ nombre, logo, descripcion, bullets = [], links = [] }) {
  ...
  return (
    ...
    >
      {/* Header */}
      <button
        className="w-full flex items-center gap-4 p-5 cursor-pointer focus:outline-none group"
        onClick={() => setOpen(v => !v)}
        aria-expanded={open}
        aria-controls={`caso-details-${nombre.replace(/\s/g, "")}`}
      >
        {/* Logo */}
        <span className="flex-shrink-0 bg-bg-light rounded-xl p-2 border border-accent-blue/40">
          <Image src={logo} alt={nombre} width={50} height={50} className="object-contain" />
        </span>
        {/* Nombre */}
        <span className="font-serif text-lg font-bold text-primary flex-1 text-left">{nombre}</span>
        {/* Flechita */}
        <motion.span
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            {/* Descripción */}
            {descripcion && <p className="mb-3 text-gray-800">{descripcion}</p>}
            {/* Bullets */}
            {bullets.length > 0 && (
              <ul className="mb-2 list-disc ml-5 text-gray-700 space-y-1">
